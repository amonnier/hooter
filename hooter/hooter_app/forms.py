...
class EnregistrementForm(forms.ModelForm):
	...
	#date = forms.DateField(required=True,widget=forms.DateInput(attrs={'placeholder': 'Date','class' : 'form-control'}),label='')
	date = forms.DateField(required=True,widget=SelectDateWidget(),label='Date de naissance')
	class Meta:
		model = Utilisateur
		fields = ['email','mot_passe','pseudo','pays','ville','date']
